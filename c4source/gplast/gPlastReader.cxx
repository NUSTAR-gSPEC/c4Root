...
gPlastReader::gPlastReader(EXT_STR_h101_gplast_onion* data, size_t offset)

    : c4Reader("gPlastReader")
    , fNEvent(0)
    , fData(data)
    , fOffset(offset)
    , fOnline(kFALSE)
    , fArray(new TClonesArray("gPlastTwinpeaksData"))
{
    c4LOG(info, Form("Reading bPlast data from %d TAMBEX card(s)", NBoards));
}
...
gPlastReader::~gPlastReader() { 
    ...
    c4LOG(info, "Destroyed gPlastReader properly.");
...
*/
Bool_t gPlastReader::Init(ext_data_struct_info* a_struct_info)
{
    Int_t ok;

    EXT_STR_h101_gplast_ITEMS_INFO(ok, *a_struct_info, fOffset, EXT_STR_h101_gplast, 0);
    ...
    FairRootManager::Instance()->Register("gPlastTwinpeaksData", "gPlastTwinpeaksDataFolder", fArray, !fOnline);
    fArray->Clear();
...
*/
void gPlastReader::DoFineTimeCalibration()
{
...
*/
double gPlastReader::GetFineTime(int tdc_fine_time_channel, int board_id, int channel_id)
{
...
*/
void gPlastReader::WriteFineTimeHistosToFile()
{
    ...
    LOG(info) << Form("gPlast fine time calibrations (i.e. raw fine time histograms) written to  %s",fine_time_histo_outfile.Data());
...
*/
void gPlastReader::ReadFineTimeHistosFromFile() 
{
    ...
    inputfile->Close();
    LOG(info) << Form("gPlast fine time calibration read from file: %s", fine_time_histo_infile.Data());
}
...
*/
Bool_t gPlastReader::Read() //do fine time here:
{
    ...
    wr_t = (((uint64_t)fData->gplast_ts_t[3]) << 48) + (((uint64_t)fData->gplast_ts_t[2]) << 32) + (((uint64_t)fData->gplast_ts_t[1]) << 16) + (uint64_t)(fData->gplast_ts_t[0]);
    

    for (int it_board_number = 0; it_board_number < NBoards; it_board_number++)
    { //per board:

        if (fData->gplast_tamex[it_board_number].event_size == 0) continue; // empty event skip
        ...
        gplast_last_lead_hit_struct last_tdc_hit; // initialized and reset - this keeps the information of the last lead that was seen
        ...
        //c4LOG(info,Form("event size: %i",fData->gplast_tamex[it_board_number].event_size));
        //c4LOG(info,Form("time_epoch =  %i, time_coarse = %i, time_fine = %i, time_edge = %i, time_channel = %i",fData->gplast_tamex[it_board_number].time_epoch,fData->gplast_tamex[it_board_number].time_coarse,fData->gplast_tamex[it_board_number].time_fine,fData->gplast_tamex[it_board_number].time_edge,fData->gplast_tamex[it_board_number].time_channel));
        //c4LOG(info,"Here comes data:");

       

        for (int it_hits = 0; it_hits < fData->gplast_tamex[it_board_number].event_size/4 - 3 ; it_hits++)
        {
            ...
            */
            ...
            if (fData->gplast_tamex[it_board_number].time_epoch <= it_hits) {fNevents_skipped++; continue;}
            if (fData->gplast_tamex[it_board_number].time_fine <= it_hits) {fNevents_skipped++; continue;}
            if (fData->gplast_tamex[it_board_number].time_coarse <= it_hits) {fNevents_skipped++; continue;}
            //the length of the arrays should be equal. 
            if (!(fData->gplast_tamex[it_board_number].time_coarse == fData->gplast_tamex[it_board_number].time_fine && fData->gplast_tamex[it_board_number].time_coarse == fData->gplast_tamex[it_board_number].time_epoch)) {fNevents_skipped++; continue;}
            ...
            if (fData->gplast_tamex[it_board_number].time_epochv[it_hits] != 0)
            {
                previous_epoch_word = fData->gplast_tamex[it_board_number].time_epochv[it_hits] & 0xFFFFFFF;
                //if (it_board_number == 1) c4LOG(info,Form("Found epoch for ch = %i, e = %i",next_channel,fData->gplast_tamex[it_board_number].time_epochv[it_hits] & 0xFFFFFFF));
                fNepochwordsread++;
            ...
            uint32_t channelid = fData->gplast_tamex[it_board_number].time_channelv[it_hits] & 0x7F; // 0-32
            
            //if (it_board_number == 1) c4LOG(info,Form("ch = %i, coarse = %i, edge = %i", channelid, fData->gplast_tamex[it_board_number].time_coarsev[it_hits], fData->gplast_tamex[it_board_number].time_edgev[it_hits]));
            
            if (fData->gplast_tamex[it_board_number].time_finev[it_hits] == 0x3FF) {fNevents_TAMEX_fail[it_board_number][channelid]++; continue;} // this happens if TAMEX loses the fine time - skip it
            
            // why channelid-1 and why skip 0 above. fatima does not do this?
            if (channelid != 0 && channelid != last_channel_read && !last_word_read_was_epoch){fNevents_lacking_epoch[it_board_number][channelid]++; c4LOG(debug2, Form("Event lacking epoch in channel %d",channelid)); continue;} // if the channel has changed but no epoch word was seen in between, channel 0 is always the first one so dont check if that s the case.
            ...
            bool is_leading = fData->gplast_tamex[it_board_number].time_edgev[it_hits] & 0x1;

            //Fill fine times and skip.
            if (!fine_time_calibration_set && is_leading) 
            {
                fine_time_hits[it_board_number][channelid]->Fill(fData->gplast_tamex[it_board_number].time_finev[it_hits]);
                continue;
            }

            uint32_t coarse_T = fData->gplast_tamex[it_board_number].time_coarsev[it_hits] & 0x7FF;
            double fine_T = GetFineTime(fData->gplast_tamex[it_board_number].time_finev[it_hits],it_board_number,channelid);
            ...
            //if(it_board_number == 1) c4LOG(info,fData->gplast_tamex[it_board_number].time_edgev[it_hits]);
            ...
            { 
                ...
                new ((*fArray)[fArray->GetEntriesFast()]) gPlastTwinpeaksData(
                    it_board_number,
                    channelid,
...
*/
void gPlastReader::PrintStatistics()
{
    std::ostringstream oss;
    // Print column labels
    oss << "\n";
    oss << ANSI_COLOR_GREEN << "gPlast TAMEX4 STATISTICS - boards from 0-N. TDC channels are numbered from 1-32 fast(odd)-slow(even)" << ANSI_COLOR_RESET;
    oss << "\n";
